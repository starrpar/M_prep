...
public class CostcoPrep {
    ...
    public boolean NORDValidParenCheck(String s){
        ...
        for(int i = 0; i < cArr.length; i++){
            ...
            if(cArr[i] == '(' ){
                ...
                if((numBracket + numBracketsRemoved*2 + numParensRemoved*2 + 1) == i && openers.get(openers.size() - 1) == 0){
                    System.out.println("numBracket: " + numBracket + ", " + "num: " + num + ", " + "numBracketsRemoved: " + numBracketsRemoved*2 + ", " + "numParensRemoved: " + numParensRemoved*2 + ", " + "i: " + i);
                    numParen--;
                    openers.remove(openers.size() - 1);
                    numParensRemoved++;
                } else {
                    System.out.println("FAIL - numBracket: " + numBracket + ", " + "numBracketsRemoved: " + numBracketsRemoved*2 + ", " + "numParensRemoved: " + numParensRemoved*2 + ", " + "i: " + i);
                    return false;
                }
            }
            if(cArr[i] == '{' ){
                ...
                if((numParen + numBracketsRemoved*2 + numParensRemoved*2 + 1) == i && openers.get(openers.size() - 1) == 1){
                    // System.out.println("numParen: " + numParen + ", " + "num: " + num + ", " + "numBracketsRemoved: " + numBracketsRemoved*2 + ", " + "numParensRemoved: " + numParensRemoved*2 + ", " + "i: " + i);
                    numBracket--;
                    openers.remove(openers.size() - 1);
                    numBracketsRemoved++;
                } else {
                    // System.out.println("FAIL - numParen: " + numParen + ", " + "numBracketsRemoved: " + numBracketsRemoved*2 + ", " + "numParensRemoved: " + numParensRemoved*2 + ", " + "i: " + i);
                    return false;
                }
            }
    ...
    public boolean divideArray(int[] nums) {
        ...
        //possible states: empty array, single value (but that is already ruled out by requiring
        //the length of array to be an even number, above)...
        //best approach - single pass through array (O(n)) and store number of occurrences
        ...
        for(int i = 0; i < nums.length; i++){
            ...
            if(numOccurrences > 0){
                // System.out.println("storing value for: " + nums[i] + " as numOccurrences: " + numOccurrences);
    ...
    int findFirstDuplicate(int[] intArray){
        ...
        //strategy: capture which values are duplicate / number of occurrences in a dictionary
        //keep track of 2nd occurrence of any char/digit in a dictionary (for more flexible alternatives)
        //could easily simply note when first "2nd occurrence" occurs and exit then returning index value
        //and char/digit at that location
        //
        //alternatively, capture indexes of each occurrence - could also be in a dictionary as 
            ...
            if(indexesOfValues.containsKey(intArray[i])){
                for(int j = 0; j < indexesOfValues.get(intArray[i]).size(); j++){
                    ...
                    if(j == 1 && indexesOfValues.get(intArray[i]).get(j) < lowestIndex){
                        // System.out.println("inside loop: " + i + ", " + j + ":: " + "index: " + indexesOfValues.get(intArray[i]).get(j) +
                        // ", " + "value: " + intArray[i]);
    ...
    int findLastDuplicate(int[] intArray){
        ...
        for(int i = 0; i < intArray.length; i++){
            if(indexesOfValues.containsKey(intArray[i])){
                for(int j = 0; j < indexesOfValues.get(intArray[i]).size(); j++){
                    // System.out.println(i + "," + j + " :: " + indexesOfValues.get(intArray[i]) + "..." + lowestIndex);
    ...
    public String countSequentialOccurrences1(String s) {
        ...
        for (int i = 1; i < s.length(); i++) {
            ...
            if (s.charAt(i) == currentChar) {
                ...
                // 5. otherwise, update currentChar to char at current position and reset count to 1
                //    also, append output string with count and currentChar

                // System.out.println("Character '" + currentChar + "' occurred " + count + " times sequentially.");
        ...
        //     (either a new char and value of 1 for count, or an incremented count for previous char)
    ...
    public long countSequentialOccurrences2(long num) {
        ...
        // - If currentChar not same as previous (i.e 'else'), simply write out values to aggregated string
        //   ...and reset currentChar to current value (str.charAt(i)) and count = 1
        // - Once for loop is complete, capture last values for currentChar and count into aggregated string
        //   and return aggregated string as result


        // Current design (submitted late for previous interview test - works but is inefficient)
        ...
        for (int i = 0; i < numAsCharArray.length; i++) {
            ...
            //    (INVESTIGATE: not sure why I broke out handling for negative vs. positive values)
            } else if (prevValue < 0) {
